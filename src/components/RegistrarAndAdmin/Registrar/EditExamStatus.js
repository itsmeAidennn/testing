...
const EditExamStatus = () => {
  ...
  const handleStatusChange = async (userId, status) => {
    try {
      const response = await fetch(`/api/users/${userId}/exam-status`, {
        method: 'PATCH',
        headers: {
    ...
    <div className={style.userProfileContainer}>
      ...
      <div className={style.statusButtons}>
        <button
          ...
          onClick={() => handleStatusChange(selectedUser.uniqueID, 'Pass')}
        >
          Pass
        </button>
        <button
          className={style.failButton}
          onClick={() => handleStatusChange(selectedUser.uniqueID, 'Fail')}
        >
          Fail
        </button>
      </div>
